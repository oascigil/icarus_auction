...
WARMUP_STRATEGY = 'FIFO' #'DOUBLE_AUCTION' #'HYBRID'
...
if len(SERVICE_TIMES) != N_SERVICES:
    raise RuntimeError("Incorrect size for SERVICE_TIMES.\n") 
...
vm_dist = [1, 5, 10, 20, 30, 40, 50, 60] #,4,5,6,7,8,9,10]#3range(20)
vm_dist = [30] #,4,5,6,7,8,9,10]#3range(20)

for strategy in ['DOUBLE_AUCTION']:
    for num_of_vms in vm_dist:
        experiment = copy.deepcopy(default)
        experiment['strategy']['name'] = strategy
        experiment['warmup_strategy']['name'] = 'FIFO'
        ...
        experiment['desc'] = "strategy: %s, number_of_vms: %s" % (strategy, str(num_of_vms))
